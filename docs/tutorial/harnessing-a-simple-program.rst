...
file ``square.s``. There are a number of other small assembly examples there
...
the state (initializers are another SmallWorld concept described in
:ref:`initializers`) and then load the binary code from a file and map it into
the state.
...
binary at the end of the commandline ``python3 basic_harnesss.py square.bin``.
...
We get back one hint per attempt at executing ``square.bin``.  The hint is
...
be set explicitly. We can now create a new script ``square.py`` which harnesses
