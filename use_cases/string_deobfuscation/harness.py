...
data section and creates a new binary `stredeobfus2` with decrypted
data section.  This is part of the tutorial in this directory.
...
# Load code as an elf
binfile = "strdeobfus"
code = smallworld.state.memory.code.Executable.from_elf(open(binfile, "rb"), address=0)
...
fns = [range(0x11FE, 0x1226), range(0x11A9, 0x11FD)]  # kringle_things  # kringle_thing
code.bounds = []
for b in fns:
...
print(
    "Done with deobf_strs.  Writing version of binary with decrypted data section for you"
)
