...
Open `zephyr.elf` in Binary Ninja, allow analysis to complete, and save the database as `zephyr.bndb` in the same directory. The scripts in this tutorial load exclusively from this `.bndb` file; the original ELF must be present alongside it so that Binary Ninja can resolve raw file content when the database is opened.
...
Rather than passing an open file handle, `from_bndb` accepts a filesystem path to the `.bndb` file. The platform, image base address, section layout, executable bounds, and symbol table are all derived automatically from the Binary Ninja database — no manual configuration is required.

We will use SmallWorld to harness the aptly named `smallworld_bug` function. Because the symbol table is loaded from the database, we can resolve both the entry point and the exit point by name using `get_symbol_value`.

```python
# Entry point / exit point
entry_point = code.get_symbol_value("smallworld_bug")
<<<<<<< HEAD:use_cases/rtos_demo/tutorial.md
exit_point  = 0x103E38 # End of smallworld_bug, found via reverse engineering
=======
exit_point  = 0x103DD8  # End of smallworld_bug
>>>>>>> 0dd60986 (update uses cases to show bndb use):use_cases/rtos_demo/README.md
