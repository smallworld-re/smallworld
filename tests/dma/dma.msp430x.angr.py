...
platform = smallworld.platforms.Platform(
    smallworld.platforms.Architecture.MSP430X, smallworld.platforms.Byteorder.LITTLE
)
...
cpu.r15.set(int(sys.argv[1]))
cpu.r14.set(int(sys.argv[2]))
...
class HDivModel(smallworld.state.models.mmio.MemoryMappedModel):
    ...
    def on_read(
        ...
        if addr >= self.quo_addr and addr < self.rem_addr:
            print(f"Reading quo from {addr:x}")
            self.numerator = 0
            self.denominator = 0
    ...
    def on_write(
        ...
        print(f"Writing to {addr:x}")
        if addr >= self.num_addr and addr < self.den_addr:
            print(f"Writing num to {addr:x}")
