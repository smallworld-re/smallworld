...
with open(filename, "rb") as f:
    code = smallworld.state.memory.code.Executable.from_elf(
        f, platform=platform, address=0x8000000
    )
    machine.add(code)
...
if final_cpu.d0.get() != 42:
    raise ValueError(f"Expected d0 to be 0x2a, got {final_cpu.d0}")
print("Test 1 SUCCESS")

# Test 2: Exit point in middle of code
exitpoint = entrypoint + code.get_symbol_size("main") - 4
machine.add_exit_point(exitpoint)
