...
`smallworld.instructions.instr_use_def.analyze_bytes` and compares the
...
Outcome categories per entry:
  ...
  error           analyze_bytes raised an exception
  decode          bytes did not decode to exactly one instruction

Usage:
  python tests/use_def/harness.py                 # all corpora
  python tests/use_def/harness.py --isa ppc32     # one ISA
  python tests/use_def/harness.py --json out.json # full per-entry report
  python tests/use_def/harness.py -v              # per-entry detail on stdout
"""
...
# instr_use_def currently contains live breakpoint() calls, and logs every
...
def canon_actual(op):
    """Operand object from analyze_bytes -> canonical tuple."""
...
def check_entry(entry, corpus, analyze_bytes):
    isa = corpus["isa"]
    ...
    try:
        analyzed = analyze_bytes(raw, lang, base)
...
def main(argv=None):
    ...
    for corpus in corpora:
        ...
        for entry in corpus["entries"]:
            results.append(check_entry(entry, corpus, analyze_bytes))
