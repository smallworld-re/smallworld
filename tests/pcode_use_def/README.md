...
instruction use/def analysis (`smallworld/instructions/pcode_use_def.py`,
`analyze()`), plus a harness (`harness.py`) that runs every corpus entry
...
- `harness.py` — runs the corpora through `analyze` and reports
  agreement with ground truth. Run as
  `python tests/pcode_use_def/harness.py [--isa x86_64] [--json report.json]`.
- `test.py` — unittest wrapper: the corpus regression (every
...
- `CORPUS.md` — a human-readable rendering of all corpora (one
  ...
  not hand-edit. Regenerate with `python tests/pcode_use_def/render_corpus.py`
