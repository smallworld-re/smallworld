...
platform = smallworld.platforms.Platform(
    smallworld.platforms.Architecture.M68K, smallworld.platforms.Byteorder.BIG
)
...
fsetpos_model = smallworld.state.models.Model.lookup(
    "fsetpos", platform, smallworld.platforms.ABI.SYSTEMV, 0x10000
)
machine.add(fsetpos_model)
fsetpos_model.allow_imprecise = True

# Relocate puts
code.update_symbol_value("fsetpos", fsetpos_model._address)
...
fgetpos_model = smallworld.state.models.Model.lookup(
    "fgetpos", platform, smallworld.platforms.ABI.SYSTEMV, 0x10010
)
...
# We signal failure fsetposs by dereferencing 0xdead.
...
try:
    ...
    pass
res = fdmgr.get_file("/tmp/foobar")
res.seek(0)
expected = b"bazgorp"
actual = res.read()

if expected != actual:
    raise Exception(f"Expected /tmp/foobar to contain {expected!r}; got {actual!r}")
