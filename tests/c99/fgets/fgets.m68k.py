...
fgets_model = smallworld.state.models.Model.lookup(
    "fgets", platform, smallworld.platforms.ABI.SYSTEMV, 0x10000
)
machine.add(fgets_model)
fgets_model.allow_imprecise = True

# Relocate puts
code.update_symbol_value("fgets", fgets_model._address)
...
# Create a fake stdin
# i386 references the copy in libc, so we need to make our own
fake_stdin = smallworld.state.memory.Memory(0x80000, 8)
...
# We signal failure fgetss by dereferencing 0xdead.
