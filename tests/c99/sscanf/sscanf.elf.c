...
#define TEST(fmt, src, arg, exp, err) \
    do {\
        ...
        if (res != 1) {\
            ...
            puts("Format");\
            puts(fmt);\
            printf("Actual does not equal actual: ");\
            printf(err, arg);\
            bad = 1;\
        } else if (exp != exp) { \
            puts("Format");\
            puts(fmt);\
            printf("Expected does not equal expected: ");\
            printf(err, exp);\
            bad = 1;\
        } else if (arg != exp) { \
            puts("Format:"); \
...
#define TEST_STR(fmt, src, arg, exp) \
    do {\
        res = sscanf(src, fmt, &arg); \
        if (res != 1) {\
            puts("Format");\
...
int main() {
    ...
    TEST_STR("%s", "foo", strval, "foo");
    TEST_STR("%s", " foo", strval, "foo");
    TEST_STR("%3s", "foobar", strval, "foo");
